        ...
        <div id="editStatus" v-if="status == 1">
            <div id="space">
                ...
                <div id="examAnswer" v-if="this.examStatus" style="margin-top: 20px">
                    答案:
                    <el-select v-model="this.correctAnswer" multiple style="width: 200px; margin: 0 8px" placeholder="正确选项序号">
                        <el-option
                        v-for="item in 10"
                        :key="item.index"
                        :value="item">
        ...
        <div id="showStatus" v-else>
            ...
            <ul>
                <li v-for="str in theChoices" :key="str.index">
                    <input type="checkbox" :class="this.number+''" :name="this.number+''" @click="write" :disabled="this.readOnly">&nbsp;&nbsp;&nbsp;{{str}}
                </li>
            </ul>
        </div>
    </div>
...
export default {
    ...
    methods:{
        ...
        write(){
            ...
            this.$emit('getResult', ans, this.number)
        },
        addOption(){
            if(this.i == 10){
                this.$message({
        ...
        confirm(){
            ...
            if(this.examStatus){
                for(var i = 0; i < this.i; i ++){
                    if(this.correctAnswer[i] > this.i){
                    this.$message({
